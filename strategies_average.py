...
def get_evaluation_results(labels: np.ndarray, trn_y, prd_trn, tst_y, prd_tst):
    ...
    # First is train and then test - accuracy, recall, specificity, precision
    # evaluation = [
    #     [(tn_trn + tp_trn) / (tn_trn + tp_trn + fp_trn + fn_trn),
    #      tp_trn / (tp_trn + fn_trn),
    #      tn_trn / (tn_trn + fp_trn),
    #      tp_trn / (tp_trn + fp_trn)],
    #     [(tn_tst + tp_tst) / (tn_tst + tp_tst + fp_tst + fn_tst),
    #      tp_tst / (tp_tst + fn_tst),
    #      tn_tst / (tn_tst + fp_tst),
    #      tp_tst / (tp_tst + fp_tst)]
    # ]
    return evaluation


def add_average(to_add, n, receiver):
    for measure in to_add:
        if measure not in receiver.keys():
            receiver[measure] = [0, 0]
        for t_set, val in enumerate(to_add[measure]):
            receiver[measure][t_set] += val / n
    return receiver
...
def naiveBayes(data, target, kfold=True, seed=None, balancing=None):
    ...
    if (balancing != None):
        df = pd.DataFrame(data=np.hstack((X, np.array([Y]).T)), columns=data.columns)
        df = balance(df, balancing, target)
        Y = df.pop(target).to_numpy().T
        X = df.to_numpy()
    ...
    if kfold:
        ...
        for train_index, test_index in skf.split(X, y):
            ...
            for clf in estimators:
                ...
                res = get_evaluation_results(labels, y_train, prd_trn, y_test, prd_tst)
                if clf not in results.keys():
                    results[clf] = {}
                results[clf] = add_average(res, nfolds, results[clf])
        return results
    else:
...
splits = naiveBayes(get_hf_data(), 'DEATH_EVENT', seed=0)
print(splits)
...
def decisionTrees(data, target, kfold=True, quick=False, seed=None, balancing=None):
    ...
    pbar.close()
    plt.show()
    print('Best results achieved with %s criteria, depth=%d and min_impurity_decrease=%1.3f ==> accuracy=%1.3f' % (
...
def randomForests(data, target, kfold=True, quick=False, seed=None, balancing=None):
    ...
    pbar.close()
    plt.show()
    print('Best results with depth=%d, %1.2f features and %d estimators, with accuracy=%1.4f' % (
        best[0], best[1], best[2], last_best))
    showConfusionMatrix(trnX, tstX, y, trnY, tstY, best_tree)
